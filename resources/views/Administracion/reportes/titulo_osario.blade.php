	...
	*{
	...
	<body>
		...
		<div id="contenido">
			...
			<p>Que el Señor: <span class="underline negrita">{{ strtoupper($datos->COMPRADOR_OSARIO) }}</span>, identificado con cedula de ciudadanía N° {{ $datos->cedula_comprador }} de {{ $datos->Municipio->nom_municipio }}-{{ $datos->Municipio->Departamento->nom_departamento }}, se responsabiliza del arriendo a perpetuidad el Osario No. <span class="underline negrita">{{ $datos->NUMERO_OSARIO }}</span> en la Cripta del Templo, para depositar los restos mortales de: <span class="underline negrita">{{ strtoupper($datos->FALLECIDO_OSARIO) }}.</span></p>
			<p id="fechas">
			...
			<p>
				TENIENDO EN CUENTA EL LUGAR “SANTO”, EL COMPRADOR SE COMPROMETE A GUARDAR EL DEBIDO RESPETO EN LA CRIPTA Y A MANTENER EN PERFECTO ESTADO EL OSARIO Y A CANCELAR LA CUOTA ANUAL DE ADMINISTRACION DE $ 30.000 PESOS.
			</p>
			<p>
			EL NO PAGO DE LA ADMINISTRACIÓN Y MANTENIMIENTO  POR UN PERIODO SUPERIOR DE 36 MESES AUTORIZA A LA PARROQUIA A DISPONER DE LAS CENIZAS O LOS RESTOS GUARDÁNDOLOS EN UN SITIO DIGNO COMÚN.  UNA VEZ SE PONGA AL DIA SE LE ASIGNA UN NUEVO OSARIO O CENIZARIO. </p>
			<p>
				EL USO DEL OSARIO ES UNIPERSONAL,  NO SE PUEDE COLOCAR FLORES, FOTOS, VELADORAS, IMÁGENES Y NINGÚN OTRO ACCESORIO PARA MANTENER LA UNIFORMIDAD.
			</p>
			<p>
