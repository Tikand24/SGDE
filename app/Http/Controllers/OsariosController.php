...
use App\Models\Administrativos\Osario\Osarios;
...
class OsariosController extends Controller
{
    ...
    {
        $osarios = Osarios::buscar($request->name)->orderBy('id', 'DESC')->paginate(50);
    ...
    public function complementos()
    {
        try {
            return response()->json([
                'ciudades' => Municipios::with(['Departamento'])->get()
            ]);
        } catch (Exception $e) {
    ...
    public function actualizarOsario(Request $request)
    {
        try {
            ...
            $osario = Osarios::find($request->id);
            $osario->NUMERO_OSARIO = $request->numero;
            $osario->COMPRADOR_OSARIO = $request->comprador;
            $osario->FALLECIDO_OSARIO = $request->fallecido;
            $osario->cedula_comprador = $request->cedula_comprador;
    ...
    public function validarNumeroOsario(Request $request)
    {
        try {
            return response()->json([
                'osario' => Osarios::where('NUMERO_OSARIO', $request->numero)->count()
            ]);
        } catch (Exception $e) {
            return response()->json($e->getMessage());
        }
    ...
    public function datosOsario(Request $request)
    {
        try {
            return response()->json([
                'osario' => Osarios::where('id', $request->id)->first()
            ]);
        } catch (Exception $e) {
            return response()->json($e->getMessage());
        }
    ...
    {
        try {
            $datos = Osarios::where('id', $id)->with(['Municipio'])->first();
