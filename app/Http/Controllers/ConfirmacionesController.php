...
use App\Models\Sacramentos\Confirmacion\AnotacionesConfirmaciones;
...
class ConfirmacionesController extends Controller
{
    ...
    {
        $confirmaciones = Confirmaciones::buscar($request->name)->orderBy('id', 'DESC')->paginate(50);
    ...
    {
        try {
            $confirmacion = Confirmaciones::where('id', $request->id)->with(['Parroquia', 'GrupoConfirmacion.CelebranteParroquia.Celebrante'])->first();
            $anotaciones = AnotacionesConfirmaciones::where('confirmacion_id', $request->id)->get();
    ...
    public function guardar(Request $request)
    {
        try {
            ...
            $confirmacion = new Confirmaciones();
    ...
    public function updated(Request $request)
    {
        ...
        try {
            ...
            $confirmacion = Confirmaciones::where('id', $request->id)->first();
            $confirmacion->nombre = $request->nombre;
            $confirmacion->libro = $request->libro;
            $confirmacion->folio = $request->folio;
            $confirmacion->partida = $request->partida;
            ...
            if ($request->tipoEdicion == "true") {
                ...
                $cambioSistema->save();
                $anotacion = new AnotacionesConfirmaciones();
    ...
    {
        try {
            return response()->json(['grupos' => GruposConfirmaciones::buscar($request->descripcion)->with(['CelebranteParroquia.Celebrante'])->get()]);
        } catch (Exception $e) {
            return response()->json($e->getMessage());
        }
    }
    ...
    {
        try {
            $datos = Confirmaciones::where('id', $id)->with(['GrupoConfirmacion.CelebranteParroquia.Celebrante', 'Parroquia.Municipio.Departamento'])->first();
            $anotaciones = AnotacionesConfirmaciones::where('confirmacion_id', $id)->get();
    ...
    {
        try {
            $anotacion = AnotacionesConfirmaciones::find($request->id);
            $cambioSistema = new CambiosSistema();
