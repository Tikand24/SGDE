...
class HomeController extends Controller
{
    ...
    public function horarioSemanal($dia)
    {
        try {
            ...
            return response()->json([
                'dia1' => HorarioEucaristias::where('dia_eucaristia_id', $hoy)
                    ->where('estado', 'Activo')
                    ->where('semanal', '1')
                    ->with(['DiasEucaristia', 'LugarEucaristia'])
                    ->get(),
                'dia2' => HorarioEucaristias::where('dia_eucaristia_id', $manhiana)
                    ->where('estado', 'Activo')
                    ->where('semanal', '1')
                    ->with(['DiasEucaristia', 'LugarEucaristia'])
                    ->get(),
                'dia3' => HorarioEucaristias::where('dia_eucaristia_id', $pasadomanhiana)
                    ->where('estado', 'Activo')
                    ->where('semanal', '1')
                    ->with(['DiasEucaristia', 'LugarEucaristia'])
                    ->get()
    ...
    public function avisosParroquiales()
    {
        try {
            return response()->json([
                'avisos' => AvisosParroquiales::orderBy('updated_at', 'desc')->limit(3)->get()
            ]);
        } catch (Exception $e) {
            return response()->json($e->getMessage());
        }
    ...
    public function allAvisosParroquiales()
    {
        try {
            return response()->json([
                'avisos' => AvisosParroquiales::orderBy('updated_at', 'desc')->get()
            ]);
        } catch (Exception $e) {
            return response()->json($e->getMessage());
        }
    ...
    {
        try {
            $horarios = HorarioEucaristias::where('estado', 'Activo')
                ->where('semanal', '1')
                ->groupBy('dia_eucaristia_id')
                ->orderBy('dia_eucaristia_id', 'asc')
                ->with(['DiasEucaristia', 'LugarEucaristia'])
                ->get();
            $horarioFinal = [];
            foreach ($horarios as $hora) {
                $horarioFinal[] = [
                    ...
                    'horario' => HorarioEucaristias::where('dia_eucaristia_id', $hora->dia_eucaristia_id)
                        ->where('estado', 'Activo')
                        ->where('semanal', '1')
                        ->orderBy('hora_eucaristia', 'asc')
                        ->with(['DiasEucaristia', 'LugarEucaristia'])
                        ->get()
                ];
            }
    ...
    public function guardarMensajeFeligres(Request $request)
    {
        try {
            ...
            $mensajeSacerdote = new MensajesSacerdotes();
    ...
    {
        $clients = array(
            ['nombre' => 'EFRAIN HERNANDEZ RAMIREZ', 'identificacion' => 9652855, 'phone' => 'hernandezyopal@gmail.com', 'tipoDoc' => 'CC', 'state' => 0],
